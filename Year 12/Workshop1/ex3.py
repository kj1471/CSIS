...
class Dog(DrawableObject):
    def __init__(self, left, top, width, height, colour, left_key, right_key):
        self.left = left
        self.top = top
        self.width = width
        self.height = height
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(surface, self.colour,
                           (  # x, y
                               self.left + body_width,
                               body_top + self.head_offset
                           ), head_rad)
        pygame.draw.rect(surface, self.colour,
                         (  # left, top, width, height:
    ...
    def update(self):
        time_passed = 1 / FRAMES_PER_SECOND
        self.time += time_passed
        # So this will flow between -1 and 1 over time
        phase = math.sin(self.BOB_SPEED * self.time)
        max_head_bob = self.height / 12  # 1/12th of the height
        self.head_offset = int(phase * max_head_bob)

    def on_event(self, event):
        if event.type == pygame.KEYUP:
            key_pressed = event.key
            if key_pressed == self.left_key:  # pygame.K_a or key_pressed == pygame.K_LEFT:
                self.left -= self.STEP_SIZE
                if self.width > 0:
                    self.left += self.width
                    self.width *= -1
            if key_pressed == self.right_key:  # pygame.K_d or key_pressed == pygame.K_RIGHT:
                self.left += self.STEP_SIZE
                if self.width < 0:
                    self.left += self.width
                    self.width *= -1
...
class ExampleGameThree(DrawableObject):
    ...
    def __init__(self):
        ...
        self.children = [
            ...
            Dog(400, 500, 150, 100, (64, 32, 16), pygame.K_a, pygame.K_d),
            Dog(600, 500, 150, 100, (128, 128, 96), pygame.K_LEFT, pygame.K_RIGHT),
        ]
